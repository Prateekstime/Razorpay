...
export default function Hero() {
  ...
  // SMOOTH AUTO SLIDER — every 4 sec
  useEffect(() => {
  ...
  const handleSlideChange = () => {
    setDirection(newIndex > currentSlide ? 0 : 1)
    setCurrentSlide(newIndex)
  }

  const slide = carouselSlides[currentSlide]
  const nextSlide = carouselSlides[(currentSlide + 1) % carouselSlides.length]
    ...
    <section className="relative min-h-screen pt-8 md:pt-18 pb-12 overflow-hidden bg-gradient-to-b from-white to-blue-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center relative">
          ...
          <div 
            className={`space-y-6 md:space-y-8 z-10 transition-all duration-1000 transform ${
              direction === 0 
          ...
          <div 
            className={`space-y-6 md:space-y-8 z-5 absolute top-0 left-0 transition-all duration-1000 transform ${
              direction === 0 
                ? 'translate-x-full opacity-0' 
                : 'translate-x-0 opacity-100'
            }`}
          ...
          >
            ...
            <div className="relative overflow-hidden">
              <h1 className="text-2xl md:text-3xl lg:text-4xl xl:text-4xl font-bold text-slate-900 leading-tight">
                {nextSlide.title}{" "}
                <span className="text-blue-600">{nextSlide.subtitle}</span>
              </h1>
            </div>
            ...
            <div className="flex items-center gap-4">
              <button className="p-2 rounded-full border-2 border-blue-600 text-blue-600 hover:bg-blue-50 transition-transform hover:scale-110">
                <ChevronLeft className="w-5 h-5" />
              </button>
              ...
              <div className="flex gap-2">
                {carouselSlides.map((_, index) => (
                  <div
                    key={index}
                    className={`h-3 rounded-full transition-all duration-300 ${
                      index === (currentSlide + 1) % carouselSlides.length
                        ? "bg-blue-600 w-8 scale-110"
                        : "bg-blue-200 w-3"
                    }`}
                  />
          ...
          <div className="relative h-64 md:h-96 lg:h-[500px] rounded-3xl overflow-hidden">
            ...
            <div 
              className={`absolute inset-0 bg-gradient-to-br from-blue-100 to-cyan-50 border border-blue-100 shadow-xl rounded-3xl overflow-hidden transition-all duration-1000 transform ${
                direction === 0 
